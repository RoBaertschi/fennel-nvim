...
local function lsp_picker(scope)
  local function _2_()
    return MiniExtra.pickers.lsp({scope = scope})
  end
  return _2_
end
local function _3_(ev)
  _G.assert((nil ~= ev), "Missing argument ev on /home/robin/.config/nvim/lua/config/lsp.fnl:18")
  do
    local map
    local function _4_(keys, func, desc, _3fmode)
      _G.assert((nil ~= desc), "Missing argument desc on /home/robin/.config/nvim/lua/config/lsp.fnl:19")
      _G.assert((nil ~= func), "Missing argument func on /home/robin/.config/nvim/lua/config/lsp.fnl:19")
      _G.assert((nil ~= keys), "Missing argument keys on /home/robin/.config/nvim/lua/config/lsp.fnl:19")
    ...
    map("gO", lsp_picker("document_symbol"), "[D]ocument [S]ymbols")
    map("gW", lsp_picker("workspace_symbol"), "[W]orkspace [S]ymbols")
    map("grt", lsp_picker("type_definition"), "Type [D]efinition")
    ...
    if (client and client_supports_method(client, vim.lsp.protocol.Methods.textDocument_documentHighlight, ev.buf)) then
      ...
      local function _5_(ev0)
        _G.assert((nil ~= ev0), "Missing argument ev on /home/robin/.config/nvim/lua/config/lsp.fnl:50")
        vim.lsp.buf.clear_references()
        return vim.api.nvim_clear_autocmds({group = "vimrc-lsp-highlight", buffer = ev0.buf})
      end
      vim.api.nvim_create_autocmd("LspDetach", {group = vim.api.nvim_create_augroup("vimrc-lsp-detach", {clear = true}), callback = _5_})
    else
    end
...
local lsps = {"lua_ls", "rust_analyzer", "ts_ls", "fennel_ls", "angularls", "clangd", "zls", "ols", "gopls", "templ"}
...
require("blink.cmp").setup({keymap = {preset = "none", ["<C-space>"] = {"show", "show_documentation", "hide_documentation"}, ["<C-e>"] = {"hide", "fallback"}, ["<C-y>"] = {"select_and_accept", "fallback"}, ["<Up>"] = {"select_prev", "fallback"}, ["<Down>"] = {"select_next", "fallback"}, ["<C-p>"] = {"select_prev", "fallback_to_mappings"}, ["<C-n>"] = {"select_next", "fallback_to_mappings"}, ["<C-b>"] = {"scroll_documentation_up", "fallback"}, ["<C-f>"] = {"scroll_documentation_down", "fallback"}, ["<C-k>"] = {"show_signature", "hide_signature", "fallback"}}, appearance = {nerd_font_variant = "mono"}, completion = {documentation = {auto_show = true, auto_show_delay_ms = 500}}, sources = {default = {"lsp", "buffer", "path", "snippets"}}, snippets = {preset = "mini_snippets"}, fuzzy = {implementation = "prefer_rust_with_warning"}, signature = {enabled = true}})
return nil
