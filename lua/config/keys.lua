...
kset("n", "<leader>tt", _1_, {desc = "Open new [T]erminal in [T]ab"})
local function _2_()
  return builtin.files()
end
kset("n", "<leader>sf", _2_, {desc = "[S]earch [F]iles"})
local function _3_()
  return builtin.grep_live()
end
kset("n", "<leader>sg", _3_, {desc = "[S]earch [G]rep"})
local function _4_()
  return builtin.buffers()
end
kset("n", "<leader>sb", _4_, {desc = "[S]earch [B]uffers"})
local function _5_()
  return builtin.help()
end
kset("n", "<leader>sh", _5_, {desc = "[S]earch [H]elp Tags"})
local function _6_()
  return builtin.resume()
end
kset("n", "<leader>sr", _6_, {desc = "[S]earch [R]esume"})
local function _7_()
  return builtin.files({}, {source = {cwd = vim.fn.stdpath("config")}})
end
kset("n", "<leader>sn", _7_, {desc = "[S]earch [N]eovim files"})
...
do
  local _8_ = io.popen("odin root", "r")
  if (nil ~= _8_) then
    local file = _8_
    ...
    kset("n", "<leader>sof", _9_, {desc = "[S]earch [O]din [F]iles"})
    local function _10_()
      return builtin.grep_live({}, {source = {cwd = odin_root, prompt_title = "Grep Odin Files"}})
    end
    kset("n", "<leader>sog", _10_, {desc = "[S]earch [O]din [G]rep"})
  elseif (_8_ == nil) then
  else
  end
end
...
if loop.fs_stat("/usr/src/linux") then
  local function _12_()
    ...
    kset("n", "<leader>slf", _13_, {desc = "[S]earch [L]inux [F]iles"})
    local function _14_()
      return builtin.grep_live({}, {source = {cwd = "/usr/src/linux/", prompt_title = "Grep Linux Files"}})
    end
    return kset("n", "<leader>slg", _14_, {desc = "[S]earch [L]inux [G]rep"})
  end
  _12_()
else
end
...
vim.api.nvim_create_user_command("OverseerRestartLast", _16_, {})
...
kset("n", "<leader>cs", _18_, {desc = "[C]onfig [S]ource"})
...
kset("n", "<leader>cfs", _19_, {desc = "[C]onfig [F]ile [S]ource"})
kset("n", "<leader>f", MiniFiles.open, {desc = "[F]iles"})
local function _20_()
  return MiniFiles.open(nil, false)
end
kset("n", "<leader>pf", _20_, {desc = "[P]roject [F]iles"})
...
kset("n", "<C-e>", _21_, {desc = "List all Harpoon files."})
local function _25_()
  return harpoon:list():add()
end
kset("n", "<leader>a", _25_, {desc = "[A]dd to Harpoon list."})
local function _26_()
  return harpoon:list():select(1)
end
kset("n", "<C-h>", _26_, {desc = "Select item 1 in the Harpoon list."})
local function _27_()
  return harpoon:list():select(2)
end
kset("n", "<C-j>", _27_, {desc = "Select item 2 in the Harpoon list."})
local function _28_()
  return harpoon:list():select(3)
end
kset("n", "<C-k>", _28_, {desc = "Select item 3 in the Harpoon list."})
local function _29_()
  return harpoon:list():select(4)
end
kset("n", "<C-l>", _29_, {desc = "Select item 4 in the Harpoon list."})
local function _30_()
  return harpoon:list():prev()
end
kset("n", "<C-S-P>", _30_, {desc = "Select next item in the Harpoon list."})
local function _31_()
  return harpoon:list():next()
end
kset("n", "<C-S-N>", _31_, {desc = "Select previous item in the Harpoon list."})
return nil
