    ...
    if (language and (language ~= "odin")) then
      ...
      if and_17_ then
        local function _18_(item)
          _G.assert((nil ~= item), "Missing argument item on /home/robin/.config/nvim/lua/config/plugins.fnl:139")
...
require("conform").setup({format_on_save = _24_, formatters_by_ft = {lua = {"stylua"}}, notify_on_error = false})
local function _26_()
